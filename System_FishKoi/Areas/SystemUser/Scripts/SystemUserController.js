const SystemUserController = function ($http, $scope, $async, $timeout, $compile) {
    const URL_REQUEST = "/SystemUser/SystemUser"
    ...
    $scope.search = () => {
        if (!isNullOrEmpty(_myDataTable)) {
            ...
            _myDataTable = $('#kt_system_user_table').DataTable({
                serverSide: true,
                processing: true,
                ordering: false,
                columns: [
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            ...
                            return `<a href="javascript:;" ng-click="showModal(${json})">${row.Username}</a>`
                        }
                    },
                    {
                        targets: 3,
                	...
                	<div class="menu-item px-3">
                		<a  ng-click="delete(${row.UserID},$event)" class="menu-link px-3">Xóa</a>
                	</div>

                    <div class="menu-item px-3">
                		<a  ng-click="updateStatus(${row.UserID},${row.Status},$event)" class="menu-link px-3">${row.Status == 1 ? 'Khoá' : 'Mở khoá'}</a>
                	</div>
                </div>`
                            return html;
                        }
            ...
            _myDataTable.on('draw', function () {
                $timeout(() => {
                    let tableElement = $('#kt_system_user_table')
    ...
    $scope.showModal = (item) => {
        if (isNullOrEmpty(item)) {
            ...
            $scope.objSystemUser = Object.assign({}, item);
        }
        $('#kt_modal_system_user').modal('show');
    }

    $scope.updateStatus = $async(async (userID, status, $event) => {
        $event.preventDefault();

        const { value } = await Swal.fire({
            text: `Bạn có chắc chắn ${status == 1 ? 'khoá' : 'mở khoá'} dữ liệu nhân viên này không?`,
            icon: "warning",
    ...
    $scope.insertOrUpdate = $async(async ($event) => {
        ...
        try {
            let action = "/Insert"
            if ($scope.objSystemUser.UserID > 0) {
                action = "/Update";
            }
            const response = await $http.post(URL_REQUEST + action, $scope.objSystemUser).then(success => success.data);
            if (response.Status === MESSAGE_STATUS.success) {
                $('#kt_modal_system_user').modal('hide');
                swalCommon("success", "Cập nhật dữ liệu thành công", false).then(() => {
                    $scope.search();
                });
            } else {
            ...
            toastr.error('Lỗi cập nhật dữ liệu nhân viên hệ thống');
        }
    })

    $scope.delete = $async(async (userID, $event) => {
        $event.preventDefault();

        const { value } = await Swal.fire({
            text: "Bạn có chắc chắn xóa dữ liệu nhân viên này không?",
            icon: "warning",
            showCancelButton: true,
            buttonsStyling: false,
            confirmButtonText: "Xác nhận",
...
SystemUserController.$inject = ["$http", "$scope", "$async", "$timeout", "$compile"];